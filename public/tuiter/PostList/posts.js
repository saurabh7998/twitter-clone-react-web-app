export default [
    {
        avatarImage: '../../images/elon_musk.png',
        userName: 'Elon Musk',
        handleName: '@elonmusk',
        time: '23h',
        heading: 'Amazing show about @Inspiration4x mission',
        contentImage: '../../images/inspiration_4x.png',
        contentTitle: 'Countdown: Inspiration4 Mission to Space | \n'
                      + 'Netflix Official Site',
        content: 'From Training to launch to landing, this all-access docuseries'
                 + ' rides along with the inspiration4  crew on the first'
                 + ' all-civillian orbital space ...',
        replies: '4.2K',
        retuits: '3.5K',
        likes: '37.5K',
    },
    {
        avatarImage: '../../images/newyorkpost.png',
        userName: 'New York Post',
        handleName: '@nypost',
        time: '23h',
        heading: 'Grimes seen reading Karl Marx following split with'
                 + ' world\'s richest man Elon Musk',
        contentImage: '../../images/grimes.png',
        contentTitle: '',
        content: '',
        replies: '965',
        retuits: '2.4K',
        likes: '4K',
    }
]